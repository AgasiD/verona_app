...
class TareaService extends ChangeNotifier {
  ...
  Future<dynamic> obtenerTareasExtras(etapaId, subetapaId, obraId) async {
    final response =
        await this._http.get('$_endpoint/tareas/extras/$etapaId/$subetapaId/$obraId');
  ...
  Future<dynamic> grabar(Map<String, dynamic> tarea) async {
    final response = await this._http.put('api/obras/agregarTarea', tarea);
    final data = json.decode(response.body);
  ...
  Future<dynamic> actualizarTarea(Tarea tarea) async {
    final response = await this._http.put('$_endpoint/tareas', tarea.toJson());
    final data = json.decode(response.body);
